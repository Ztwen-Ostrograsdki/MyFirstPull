<?php
namespace MyFramework\HTMLFormat;
...
class Form{
    ...
    public function setMultiSelect (array $options, $default = null, $width = 70):string
    {
       $optionHTML = [];
        ...
        return "<div class='p-0 mx-0 mt-2'>
            <div class='register-table'>
                <div class='label'>
                    <label for = '".$this->name. "' class = 'ml-2'>{$this->label}</label>
                </div>
               <div class='input'>
                    <select multiple name = '".$this->name."' value = '".$option."' id = '".$this->name."' class = '".$this->classListAdvanced."'  required style = 'width:".$width."%;'>{$optionHTML}</select>  
               </div>          
            </div>
        </div>
        ";
    ...
    public function setUniqueSelect (array $options, $default = null, $width = 70):string
    {
       $optionHTML = [];
        foreach ($options as $option) {
            if ($default !== null && $default === $option) {
                $selected = 'selected';
            }
            else{
                $selected = '';
        ...
        return "<div class='p-0 mx-0 mt-2'>
            <div class='register-table'>
               ...
               <div class='input'>
                    <select name = '".$this->name."' value = '".$this->getValue()."' id = '".$this->name."' class = '".$this->classListAdvanced."'  required style = 'width:".$width."%;'>{$optionHTML}</select>  
               </div>          
            </div>
        </div>
        ";
    ...
    public function advancedSetInputRadio(array $values, $default = null, $width = 20):string
    {
        $optionHTML = [];
        foreach ($values as $key => $option) {
            ...
            $optionHTML[] = "<label for = '{$option}'>{$option}</label><input type = 'radio' class = '".$this->classListAdvanced."' name = '".$this->getName()."' id = '".$option."' value = '" .$option."' style = 'width:".$width."%;' $this->disabled required $checked>";
        }
        $optionHTML = implode('', $optionHTML);
        return "<div class='p-0 mx-0 mt-2'>
            <div class='register-table'>
                <div class='label'>
                    <label class = 'ml-2'>{$this->label} :</label>
                </div>
                {$optionHTML}                
            </div>
        </div>
